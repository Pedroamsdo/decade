...
def _build_base(
    ...
    sub = df_subclasse.select(
        ...
        text_strip_expr("Denominacao_Social", "denom_social_subclasse"),
...
def _read_anbima_subclasse(settings: Settings) -> pl.DataFrame:
    ...
    if not candidates:
        log.warning("silver.subclass_funds.anbima_missing", drop_dir=str(drop_dir))
    ...
    codigo_sub_col = find_column(df, "Código CVM Subclasse", "Codigo CVM Subclasse")
